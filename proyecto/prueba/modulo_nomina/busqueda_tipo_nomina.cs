...
{
    public partial class busqueda_tipo_nomina : Form
    {
        public busqueda_tipo_nomina()
        {
            InitializeComponent();
        }
        ...
        private void busqueda_tipo_nomina_Load(object sender, EventArgs e)
        {
            cargar_datos();
        }
        public void cargar_datos()
        {
            try
            {
                ...
                string sql = "select codigo,nombre,estado from nomina_tipos where estado='1'";
        ...
        private void nombre_txt_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            {
                dataGridView1.Rows.Clear();
                string sql = "select codigo,nombre,estado from nomina_tipos where estado='1' and nombre like '%"+nombre_txt.Text.Trim()+"%'";
                DataSet ds = Utilidades.ejecutarcomando(sql);
                foreach (DataRow row in ds.Tables[0].Rows)
                {
                    dataGridView1.Rows.Add(row[0].ToString(), row[1].ToString(), row[2].ToString());
